...
if ac.findNodes("under_gloss_sec_sec") and 
   ac.findNodes("under_gloss_sec_sec"):findMeshes("under_gloss_sec_sec") and 
   ac.findNodes("under_gloss_sec_sec"):findMeshes("under_gloss_sec_sec"):blendMode("under_gloss_sec_sec") == 2 then
   ac.debug("Script", "✓ Скрипт активирован")
else
    ac.debug("Script", "× Скрипт отключен, несоответствие условий (спиздил скрипт)")
    ac.debug("Buy script (telegram)", " @p400_e")
    return
end
...
function Driver:initialize()
    ...
    -- Переменные для мигания (предупреждения)
    self.blinkTimer = 0
    self.blinkState = false -- false = выкл, true = вкл
    self.blinkInterval = 0.15 -- Скорость мигания 
...
function Driver:updateRadar(dt)
    ...
    for i = 0, ac.getSim().carsCount - 1 do
        if i ~= 0 then
            ...
            -- Проверяем только машины в радиусе 6.35 метров
            if distance < 6.35 then
                ...
                local isToLeft = angleDeg > 100 -- Левая зона: угол > 100°
                local isToRight = angleDeg < 80 -- Правая зона: угол < 80°
